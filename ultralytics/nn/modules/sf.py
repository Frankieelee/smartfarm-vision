...
class ScalSeq(nn.Module):
    ...
    接收来自不同尺度的多个特征图（如 P3, P4, P5），通过 3D 卷积沿尺度维度 进行特征融合，替代传统的 Concat 操作。
    ...
    def __init__(self, c1, c2, n=3, act=True):
        ...
        self.channel_align = nn.ModuleList(
            [nn.Conv2d(c_in, c2, kernel_size=1, stride=1, padding=0, bias=False) for c_in in self.c1_list]
        )

        # 3D 卷积：沿尺度维度进行跨尺度语义融合
        # kernel_size = (1, 1, 1) 表示 (尺度维度, 高度, 宽度)
        self.conv3d = nn.Conv3d(in_channels=c2, out_channels=c2, kernel_size=(1, 1, 1), stride=1, padding=0, bias=False)
        ...
        self.pool3d = nn.MaxPool3d(kernel_size=(n, 1, 1), stride=1, padding=0)
    ...
    def forward(self, x):
        ...
        for i, feat in enumerate(x):
            ...
            if feat_aligned.shape[2] != target_h or feat_aligned.shape[3] != target_w:
                feat_aligned = F.interpolate(feat_aligned, size=(target_h, target_w), mode="nearest")
