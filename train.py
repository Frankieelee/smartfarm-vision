...
def freeze_layers(model, freeze_config):
    ...
    print(
        f"   🔥 可训练参数: {trainable_params:,} ({trainable_params / (frozen_params + trainable_params) * 100:.1f}%)"
    )
    print(f"{'=' * 60}\n")
